...
if $(docker-machine env $MACHINE_NAME > /dev/null 2>&1) ; then
  ...
  if [ -z $AWS_ACCESS_KEY_ID ]; then
    docker-machine create --driver amazonec2 --amazonec2-vpc-id $VPC_ID --amazonec2-root-size 20 --amazonec2-instance-type t2.large $MACHINE_NAME 
  else
    docker-machine create --driver amazonec2 --amazonec2-access-key $AWS_ACCESS_KEY_ID --amazonec2-secret-key $AWS_SECRET_ACCESS_KEY --amazonec2-vpc-id $VPC_ID --amazonec2-root-size 20 --amazonec2-instance-type t2.large --amazonec2-region $AWS_DEFAULT_REGION $MACHINE_NAME
  fi
fi
...
#Copy Repositories from Jump Server to Docker-Machine
docker-machine scp -r /home/ec2-user/AST/sph-adop-docker-compose/gitrepos/key.git $MACHINE_NAME:~/
docker-machine scp -r /home/ec2-user/AST/sph-adop-docker-compose/gitrepos/Selenium-Server-Check.git $MACHINE_NAME:~/
docker-machine scp -r /home/ec2-user/AST/sph-adop-docker-compose/gitrepos/ast-webapp.git $MACHINE_NAME:~/

#Copy Projects to Gerrit Container
docker-machine ssh $MACHINE_NAME sudo docker cp key.git gerrit:/var/git/repos
docker-machine ssh $MACHINE_NAME sudo docker cp Selenium-Server-Check.git gerrit:/var/git/repos
docker-machine ssh $MACHINE_NAME sudo docker cp ast-webapp.git gerrit:/var/git/repos

#Restart Gerrit
docker-machine ssh $MACHINE_NAME sudo docker restart gerrit
